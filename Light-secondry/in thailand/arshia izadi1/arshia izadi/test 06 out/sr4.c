...
void sensor()
    ...
    lcd_gotoxy(12,1);
    lcd_putchar((SK/100)%10+'0');
    lcd_putchar((SK/10)%10+'0');
    lcd_putchar((SK/1)%10+'0');
...
void catch_r()
    {          
          if (imin==9)    motor(-v,-v,v,v);        //motor(-v,-v/2,v,v/2);
...
void catch_l()
    {
          if (imin==1)    motor(v,0,-v,0);         //motor(v,v/2,-v,-v/2);
          ...
          else if (imin==7)    motor(-v,-v,v,v);        // motor(-v/2,-v/2,v/2,v);
...
while (1)
      ...
      sensor();  
        if(SK>600 && SB<150)
          ...
          while(f==1 &&(imin<4 || imin>12) && SB < 200)
          {                                 
          if(PINC.3==1)      CF=compass_read(1);
          sensor();      
          if(SK>600 && SB < 200)    motor(-v,-v,v,v);
          else          motor(0,0,0,0);
          }
          f=0;   
          while(SB>150 && imin>=4 && imin<=12)
          {
          sensor();
          ...
          while(SR>150 /*&& imin>=0 && imin <=8*/ )//&& min<800 )
          {                                     
          if(PINC.3==1)      CF=compass_read(1);
          sensor();
          if(SR>250)motor(-v,v,v,-v);
          else if(!(imin>=0 && imin <=8))    catch_r();     
          else motor(0,0,0,0); 
          }
          while(SL>150 && (imin>=8 || imin==0))// && min<800)
          {                                     
          if(PINC.3==1)      CF=compass_read(1);
          sensor();
          if(SL>250)motor (v,-v,-v,v);
          else if(imin>=0 && imin <=8)    catch_l();     
          else motor(0,0,0,0);
          }
